...
<head>
    ...
    <title>DATA REKAP BULANAN</title>
    <style>
        body {
            background: #000428;
...
<body>
    <div class="row header">
        <div class="col-12">
            ...
            <h1 class="brand">DATA REKAP BULANAN</h1>
        </div>
    </div>
    <div class="row content">
        <div class="col-12">
            <div class="right">
                <a href="add.php" class="btn add" data-toggle="tooltip" data-placement="top" title="Klik untuk tambah data rekap harian"><i class="bi bi-file-earmark-plus-fill"></i>Add</a>
                ...
                <table class="table table-bordered" id="myTable">
                    ...
                    <tbody>
                        ...
                        if ($_SERVER['REQUEST_METHOD'] == "POST") {
                            ...
                            if ($pencarian != '') {
                                $sql = "SELECT * FROM rekap_harian ORDER BY bulan ASC";
                                $query = $sql;
                                $queryJml = $sql;
                            } else {
                                $query = "SELECT * FROM rekap_harian ORDER BY bulan ASC LIMIT $posisi, $batas";
                                $queryJml = "SELECT * FROM rekap_harian ORDER BY bulan ASC";
                                $no = $posisi + 1;
                            }
                        } else {
                            $query = "SELECT * FROM rekap_harian ORDER BY bulan ASC LIMIT $posisi, $batas";
                        ...
                        $sql_rekapharian = mysqli_query($connect, $query) or die(mysqli_error($connect));
                        if (mysqli_num_rows($sql_rekapharian) > 0) {
                            while ($data = mysqli_fetch_array($sql_rekapharian)) { ?>
                                <tr>
                                    ...
                                    <td><?= $data['paraf']; ?></td>
                                    <td class="center">
                                        <a href="edit.php?id=<?= $data['laba'] ?>" id="edit" data-toggle="tooltip" data-placement="top" title="Klik untuk edit data" onclick="return confirm('Apakah anda yakin ingin edit data ini?')"><i class="bi bi-pencil-square"></i></a>
                                        <a href="delete.php?id=<?= $data['laba'] ?>" id="delete" data-toggle="tooltip" data-placement="top" title="Klik untuk delete data" onclick="return confirm('Apakah anda yakin ingin delete data ini?')"><i class="bi bi-trash"></i></a>
                                    </td>
                                </tr>
                        <?php
                            }
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#myTable').DataTable({
                ...
                buttons: [{
                        extend: 'pdf',
                        title: 'DATA REKAP BULANAN GUDANG PT. AMSKY INDONESIA',
                        download: 'open',
