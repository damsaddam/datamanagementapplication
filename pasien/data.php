...
<body>
    <div class="row header">
        <div class="col-12">
            ...
            <h1 class="brand">DATA PASIEN</h1>
        </div>
    </div>
    <div class="row content">
        <div class="col-12">
            <div class="right">
                <a href="add.php" class="btn add" data-toggle="tooltip" data-placement="top" title="Klik untuk menambahkan data pasien"><i class="bi bi-file-earmark-plus-fill"></i>Add</a>
                <a href="import.php" class="btn import" data-toggle="tooltip" data-placement="top" title="Klik untuk mengimpor data pasien dari excel"><i class="bi bi-database-add"></i>Import</a>
            </div>
                    ...
                    <tbody>
                        ...
                        if ($_SERVER['REQUEST_METHOD'] == "POST") {
                            ...
                            if ($pencarian != '') {
                                ...
                                $query = "SELECT * FROM pasien ORDER BY no_rm ASC LIMIT $posisi, $batas";
                                $queryJml = "SELECT * FROM pasien ORDER BY no_rm ASC";
                                $no = $posisi + 1;
                            }
                        } else {
                            $query = "SELECT * FROM pasien ORDER BY no_rm ASC LIMIT $posisi, $batas";
                        ...
                        if (mysqli_num_rows($sql_pasien) > 0) {
                            while ($data = mysqli_fetch_array($sql_pasien)) { ?>
                                <tr>
                                    ...
                                    <td><?= $data['no_telp']; ?></td>
                                    <td class="center">
                                        <a href="edit.php?id=<?= $data['id_pasien'] ?>" id="edit" data-toggle="tooltip" data-placement="top" title="Klik untuk edit data" onclick="return confirm('Apakah anda yakin ingin edit data ini?')"><i class="bi bi-pencil-square"></i></a>
                                        <a href="delete.php?id=<?= $data['id_pasien'] ?>" id="delete" data-toggle="tooltip" data-placement="top" title="Klik untuk delete data" onclick="return confirm('Apakah anda yakin ingin delete data ini?')"><i class="bi bi-trash"></i></a>
                                    </td>
                                </tr>
                        <?php
                            }
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#myTable').DataTable({
                ...
                buttons: [{
                        extend: 'pdf',
                        title: 'DATA PASIEN KLINIK PRIMA UTAMA MEDIKA',
                        download: 'open',
