...
<body>
    <div class="row header">
        <div class="col-12">
            ...
            <h1 class="brand">DATA DOKTER</h1>
        </div>
    </div>
    <div class="row content">
        <div class="col-12">
            <div class="right">
                <a href="add.php" class="btn add" data-toggle="tooltip" data-placement="top" title="Klik untuk menambahkan data dokter"><i class="bi bi-file-earmark-plus-fill"></i>Add</a>
                <a href="import.php" class="btn import" data-toggle="tooltip" data-placement="top" title="Klik untuk mengimpor data dokter dari excel"><i class="bi bi-database-add"></i>Import</a>
            </div>
                ...
                <table class="table table-bordered" id="myTable">
                    ...
                    <tbody>
                        ...
                        if ($_SERVER['REQUEST_METHOD'] == "POST") {
                            ...
                            if ($pencarian != '') {
                                $sql = "SELECT * FROM dokter ORDER BY sip ASC";
                                $query = $sql;
                                $queryJml = $sql;
                            } else {
                                $query = "SELECT * FROM dokter ORDER BY sip ASC LIMIT $posisi, $batas";
                                $queryJml = "SELECT * FROM dokter ORDER BY sip ASC";
                                $no = $posisi + 1;
                            }
                        } else {
                            $query = "SELECT * FROM dokter ORDER BY sip ASC LIMIT $posisi, $batas";
                        ...
                        $sql_dokter = mysqli_query($connect, $query) or die(mysqli_error($connect));
                        if (mysqli_num_rows($sql_dokter) > 0) {
                            while ($data = mysqli_fetch_array($sql_dokter)) { ?>
                                <tr>
                                    ...
                                    <td class="center">
                                        <a href="edit.php?id=<?= $data['id_dokter'] ?>" id="edit" data-toggle="tooltip" data-placement="top" title="Klik untuk edit data" onclick="return confirm('Apakah anda yakin ingin edit data ini?')"><i class="bi bi-pencil-square"></i></a>
                                        <a href="delete.php?id=<?= $data['id_dokter'] ?>" id="delete" data-toggle="tooltip" data-placement="top" title="Klik untuk delete data" onclick="return confirm('Apakah anda yakin ingin delete data ini?')"><i class="bi bi-trash"></i></a>
                                    </td>
                                </tr>
                        <?php
                            }
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#myTable').DataTable({
                ...
                buttons: [{
                        extend: 'pdf',
                        title: 'DATA DOKTER KLINIK PRIMA UTAMA MEDIKA',
                        download: 'open',
